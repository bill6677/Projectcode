					...
					<div id="pan1cont" class="ibox-content"
						...
						<div class="tab-content">
							<div  id="tab-4" class="tab-pane active ">
								...
								<table id="departmentTable" class="table-hover" width="80%"></table>
							</div>
...
<script type="text/javascript">
   ...
   function  tableInit(){
		$('#departmentTable').bootstrapTable({
			url : '<%=request.getContextPath()%>/Manager/fabing.json',
			clickToSelect : true,
			singleSelect : true,
			toolbar : '#toolbar',
			toolbarAlign : '-',
			...
			columns : [/* {
				...
				align : "center"
			/* 	formatter: function(value,row,index){
					return "<a href=''>90%</a>" ;          //从数据库查出来的percent值,用toFixed进行保留两位数，然后添加上%即可
					} */
			},{
				title : '发病率(%)',
				field : 'item1',
				formatter: function(value,row,index){
					return row["item1"]==null?0:(row["item1"]).toFixed(1);
   ...
   function queryParams(params) {
	...
	function initEchart() {
		...
		var option = {
			title : {
				text : $("#reportperiod_year").val()+'年每月'+$("#reportperiod_month").val()+'发病率',
				y:'bottom',
				x:'center'
			},
			tooltip : {
